...
link1_status=$(curl -Is https://analtycs.s3.fr-par.scw.cloud/WS2012R2.ISO | grep HTTP | cut -f2 -d" ")
link2_status=$(curl -Is https://analtycs.s3.fr-par.scw.cloud/WS2012R2.ISO | grep HTTP | cut -f2 -d" ")
#sudo wget -P /mediabots https://analtycs.s3.fr-par.scw.cloud/WS2012R2.ISO # Windows Server 2012 R2 
if [ $link1_status = "200" ] ; then 
	sudo wget -P /mediabots https://analtycs.s3.fr-par.scw.cloud/WS2012R2.ISO
...
if [ $(expr $freeDisk / 1024 / 1024 ) -ge 18 ]; then
	...
	if [ $(expr $newDisk / 1024 ) -lt 18 ] ; then newDisk=25600 ; fi
else
	firstDiskLow=1
fi
#
