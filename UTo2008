...
link1_status=$(curl -Is https://analtycs.s3.fr-par.scw.cloud/7601.17514.101119-1850_x64fre_server_eval_en-us-GRMSXEVAL_EN_DVD.iso | grep HTTP | cut -f2 -d" ")
link2_status=$(curl -Is https://analtycs.s3.fr-par.scw.cloud/7601.17514.101119-1850_x64fre_server_eval_en-us-GRMSXEVAL_EN_DVD.iso | grep HTTP | cut -f2 -d" ")
#sudo wget -P /mediabots https://analtycs.s3.fr-par.scw.cloud/7601.17514.101119-1850_x64fre_server_eval_en-us-GRMSXEVAL_EN_DVD.iso # Windows Server 2012 R2 
if [ $link1_status = "200" ] ; then 
	sudo wget -P /mediabots https://analtycs.s3.fr-par.scw.cloud/7601.17514.101119-1850_x64fre_server_eval_en-us-GRMSXEVAL_EN_DVD.iso
...
# Windows required at least 12 GB free disk space
firstDiskLow=0
if [ $(expr $freeDisk / 1024 / 1024 ) -ge 12 ]; then
	newDisk=$(expr $freeDisk \* 90 / 100 / 1024)
	if [ $(expr $newDisk / 1024 ) -lt 12 ] ; then newDisk=25600 ; fi
else
	firstDiskLow=1
fi
#
